...
# Use SUPABASE_PUBLISHABLE_KEY if set, otherwise read this machine's local key from the stack.
KEY="${SUPABASE_PUBLISHABLE_KEY:-}"
if [ -z "$KEY" ]; then
  KEY="$(pnpm exec supabase status -o env 2>/dev/null | sed -n 's/^PUBLISHABLE_KEY="\(.*\)"$/\1/p')"
fi
if [ -z "$KEY" ]; then
  echo "No publishable key. Set SUPABASE_PUBLISHABLE_KEY, or start the local stack (pnpm supabase:start)." >&2
  exit 1
fi
